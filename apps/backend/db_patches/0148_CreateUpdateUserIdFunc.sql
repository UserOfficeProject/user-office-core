...
BEGIN
    ...
    BEGIN
      ...
      BEGIN
         ...
         (CAST(PROPOSAL_PK AS TEXT) || CAST(FAP_ID AS TEXT)) NOT IN ( SELECT CAST(PROPOSAL_PK AS TEXT) || CAST(FAP_ID AS TEXT) as FA_KEY 
          FROM FAP_ASSIGNMENTS WHERE fap_member_user_id = P_NEW_USER_ID );
          DELETE FROM FAP_ASSIGNMENTS WHERE FAP_MEMBER_USER_ID = P_OLD_USER_ID AND 
          (CAST(PROPOSAL_PK AS TEXT) || CAST(FAP_ID AS TEXT)) IN ( SELECT CAST(PROPOSAL_PK AS TEXT) || CAST(FAP_ID AS TEXT) as FA_KEY 
          FROM FAP_ASSIGNMENTS WHERE fap_member_user_id = P_NEW_USER_ID );
