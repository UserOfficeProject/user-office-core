...
BEGIN
  IF REGISTER_PATCH('CreateOrReplaceOldUserIdWithNewFunc.SQL', 'CHI KAI LAM', 'UPDATE USER ID FOR MERGING', '2024-03-22') THEN
    BEGIN
      ...
      INSERT INTO MERGING_TABLE_REGIESTRY (TABLE_NAME, FIELD_NAME) VALUES 
        ('EVENT_LOGS', 'CHANGED_BY'),
        ('EXPERIMENT_SAFETY_INPUTS', 'CREATOR_ID'),
        ('FAP_ASSIGNMENTS', 'FAP_MEMBER_USER_ID'),
        ('FAP_MEETING_DECISIONS', 'SUBMITTED_BY'),
        ('FAP_REVIEWERS', 'USER_ID'),
        ('FAP_REVIEWS', 'USER_ID'),
        ('FAP_SECRETARIES', 'USER_ID'),
        ('FAPS', 'FAP_CHAIR_USER_ID'),
        ('FEEDBACKS', 'CREATOR_ID'),
        ('GENERIC_TEMPLATES', 'CREATOR_ID'),
        ('INSTRUMENT_HAS_SCIENTISTS', 'USER_ID'),
        ('INSTRUMENTS', 'MANAGER_USER_ID'),
        ('INTERNAL_REVIEWS', 'ASSIGNED_BY'),
        ('INTERNAL_REVIEWS', 'REVIEWER_ID'),
        ('PDF_TEMPLATES', 'CREATOR_ID'),
        ('PREDEFINED_MESSAGES', 'LAST_MODIFIED_BY'),
        ('PROPOSAL_USER', 'USER_ID'),
        ('PROPOSALS', 'PROPOSER_ID'),
        ('QUESTIONARIES', 'CREATOR_ID'),
        ('REDEEM_CODES', 'CLAIMED_BY'),
        ('REDEEM_CODES', 'CREATED_BY'),
        ('REDEEM_CODES', 'PLACEHOLDER_USER_ID'),
        ('ROLE_USER', 'USER_ID'),
        ('SAMPLES', 'CREATOR_ID'),
        ('SCHEDULED_EVENTS', 'LOCAL_CONTACT'),
        ('SHIPMENTS', 'CREATOR_ID'),
        ('TECHNICAL_REVIEW', 'REVIEWER_ID'),
        ('TECHNICAL_REVIEW', 'TECHNICAL_REVIEW_ASSIGNEE_ID'),
        ('VISITS', 'CREATOR_ID'),
        ('VISITS', 'TEAM_LEAD_USER_ID'),
        ('VISITS_HAS_USERS', 'USER_ID');

      CREATE OR REPLACE PROCEDURE VERIFY_MERGING_TABLES()
      LANGUAGE PLPGSQL AS
      $DO$
      DECLARE
        MERGE_TABLE_CHANGED INTEGER := 0;
        MERGE_TABLE_NOT_EXIST INTEGER := 0;
      ...
      BEGIN
        ...
        WHERE TC.CONSTRAINT_TYPE = 'FOREIGN KEY'
          ...
          AND NOT EXISTS (
          SELECT MTR.TABLE_NAME, MTR.FIELD_NAME
            FROM MERGING_TABLE_REGIESTRY MTR 
            WHERE UPPER(MTR.TABLE_NAME) = UPPER(TC.TABLE_NAME) 
              AND UPPER(MTR.FIELD_NAME) = UPPER(KCU.COLUMN_NAME)
            ) 
        INTO MERGE_TABLE_CHANGED;

        IF MERGE_TABLE_CHANGED > 0 THEN 
          RAISE EXCEPTION '% TABLE(S) BEING MERGED DO NOT ALIGN WITH THOSE ADDRESSED BY THE REPLACE_OLD_USER_ID_WITH_NEW FUNCTION. KINDLY REVISE THE REPLACE_OLD_USER_ID_WITH_NEW FUNCTION ACCORDINGLY.', MERGE_TABLE_CHANGED;
        END IF; 

        SELECT COUNT(DISTINCT MTR.TABLE_NAME) FROM MERGING_TABLE_REGIESTRY MTR 
        WHERE NOT EXISTS (
          ...
          WHERE TC.CONSTRAINT_TYPE = 'FOREIGN KEY'
            ...
            AND UPPER(CCU.COLUMN_NAME) ='USER_ID'
            AND UPPER(CCU.TABLE_NAME) ='USERS'
            AND UPPER(MTR.TABLE_NAME) = UPPER(TC.TABLE_NAME) 
            AND UPPER(MTR.FIELD_NAME) = UPPER(KCU.COLUMN_NAME)
            )
        INTO MERGE_TABLE_NOT_EXIST;

        IF MERGE_TABLE_NOT_EXIST > 0 THEN 
          RAISE EXCEPTION '% TABLE(S) BEING MERGED DO NOT EXIST. KINDLY REVISE THE REPLACE_OLD_USER_ID_WITH_NEW FUNCTION ACCORDINGLY.', MERGE_TABLE_NOT_EXIST;
        END IF;
      ...
      BEGIN
          ...
          UPDATE FAP_ASSIGNMENTS AS FA1 
            SET FAP_MEMBER_USER_ID = P_NEW_USER_ID 
            WHERE FAP_MEMBER_USER_ID = P_OLD_USER_ID 
            AND NOT EXISTS (
              SELECT FA2.PROPOSAL_PK, FA2.FAP_ID 
                FROM FAP_ASSIGNMENTS FA2 
                WHERE FA2.PROPOSAL_PK = FA1.PROPOSAL_PK
                  AND FA2.FAP_ID = FA1.FAP_ID
                  AND FA2.FAP_MEMBER_USER_ID = P_NEW_USER_ID
            );

          DELETE FROM FAP_ASSIGNMENTS FA1 
            WHERE FAP_MEMBER_USER_ID = P_OLD_USER_ID 
            AND EXISTS (
              SELECT FA2.PROPOSAL_PK, FA2.FAP_ID 
                FROM FAP_ASSIGNMENTS FA2 
