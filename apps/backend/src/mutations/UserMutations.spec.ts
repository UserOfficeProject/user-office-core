...
describe('Role Mutations', () => {
  test('A normal user cannot create a role', async () => {
    const result = await userMutations.createRole(dummyUserWithRole, {
      title: 'New Role',
      shortCode: 'new_role',
  ...
  test('A user officer can create a role', async () => {
    const result = await userMutations.createRole(dummyUserOfficerWithRole, {
      title: 'New Role',
      shortCode: 'new_role',
      description: 'A new role',
      permissions: [],
  ...
  test('A normal user cannot update a role', async () => {
    const result = await userMutations.updateRole(dummyUserWithRole, {
      roleID: 1,
      title: 'Updated Role',
  ...
  test('A user officer can update a role', async () => {
    const result = await userMutations.updateRole(dummyUserOfficerWithRole, {
      roleID: 1,
      title: 'Updated Role',
      shortCode: 'updated_role',
      description: 'An updated role',
  ...
  test('A normal user cannot delete a role', async () => {
    const result = await userMutations.deleteRole(dummyUserWithRole, 1);
    expect(isRejection(result)).toBe(true);
    expect(result).toHaveProperty('reason', 'INSUFFICIENT_PERMISSIONS');
  });

  test('A user officer can delete a role', async () => {
    const result = await userMutations.deleteRole(dummyUserOfficerWithRole, 1);
    expect(isRejection(result)).toBe(false);
