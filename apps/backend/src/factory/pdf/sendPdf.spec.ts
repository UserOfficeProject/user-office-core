...
import { canRenderLocally, getPdfEngine } from './localRenderer';
...
describe('canRenderLocally', () => {
  ...
  it('accepts proposals that all carry a template', () => {
    expect(
      canRenderLocally(PDFType.PROPOSAL, [withTemplate(1), withTemplate(2)])
    ).toBe(true);
  });

  it('accepts experiment safety documents', () => {
    expect(canRenderLocally(PDFType.EXPERIMENT_SAFETY, [withTemplate(1)])).toBe(
      true
    );
  });
  ...
  it('rejects the request when any entity has no template', () => {
    expect(
      canRenderLocally(PDFType.PROPOSAL, [withTemplate(1), { proposal: {} }])
    ).toBe(false);
  });

  it('rejects a template with an empty body', () => {
    expect(
      canRenderLocally(PDFType.PROPOSAL, [
  ...
  it('rejects an empty request', () => {
    expect(canRenderLocally(PDFType.PROPOSAL, [])).toBe(false);
  });

  it('rejects types the local engine does not cover', () => {
    expect(canRenderLocally(PDFType.SAMPLE, [withTemplate(1)])).toBe(false);
    expect(canRenderLocally(PDFType.SHIPMENT_LABEL, [withTemplate(1)])).toBe(
      false
    );
  });
  ...
  it('rejects everything when the engine is pinned to the factory', () => {
    ...
    expect(canRenderLocally(PDFType.PROPOSAL, [withTemplate(1)])).toBe(false);
  });
});
