...
export function ProposalComponentTextInput(props: IBasicComponentProps) {
  ...
  return (<div>
    <TextField id={proposal_question_id} name={proposal_question_id} fullWidth label={question} value={templateField.value} onChange={(evt: ChangeEvent<HTMLInputElement>) => {
      templateField.value = evt.target.value;
      handleChange(evt); // letting Formik know that there was a change
    }} onBlur={() => onComplete()} placeholder={config.small_label} error={isError} helperText={errors[proposal_question_id]} multiline={config.multiline} rows={config.multiline ? 4 : 1} className={classes.textField} InputLabelProps={{
      shrink: true,
    }} />
  </div>);
}
