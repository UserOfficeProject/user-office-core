...
context('Event log tests', () => {
  ...
  // it('If officer uptates user info, should be able to see the event log for that update immediately', () => {
  //     const newFirstName = faker.name.firstName();
  //     //@ts-ignore
  //     cy.login('officer');
  ...
  //     /** NOTE:
  //      * Should wait some time to finish the update. Not sure if 1000 will always work.
  //      * Not a big fan of waiting for a random time.
  //      * It should be solved by waiting the request to finish. (https://docs.cypress.io/api/commands/wait.html#Aliases) but using GraphQL it could be a bit tricky.
  //      */
  //     cy.wait(1000);
  ...
  //     // NOTE: This email should not be hardcoded here. We should use email of user who did the change if we can get that info. For now it works.
  //     lastRowText.should('contain', 'Aaron_Harris49@gmail.com');
  //     lastRowText.should('contain', 'USER_UPDATED');
  //     lastRowText.should('contain', updateProfileDate);
  //   });
});
