  ...
  TestFilter(['ess'], () => {
    describe('Proposal templates basic tests', () => {
      ...
      it('User officer can delete template', () => {
        ...
        cy.contains(`Copy of ${initialDBData.template.name}`).should(
          'not.exist'
        );
      });
      ...
      it('should render the Number field accepting only positive, negative numbers if set', () => {
        ...
        cy.contains(numberQuestion3.title);
        cy.get('body').then(() => {
          cy.get(`[data-cy="${numberField1Id}.value"] input`).as(
            'numberField1'
          );
          cy.get(`[data-cy="${numberField2Id}.value"] input`).as(
            'numberField2'
          );
    ...
    describe('File upload tests', () => {
      ...
      it('Unidentifiable disguised file is uploaded but not accepted', () => {
        ...
        cy.notification({
          variant: 'error',
          text: 'not satisfying constraint',
        });
      });
